...
export default function CategoriesIndex({ categories }: Props) {
  ...
  const confirmDelete = () => {
    if (selectedCategory) {
      router.delete(route("dashboard.finance.categories.destroy", selectedCategory.id), {
        onSuccess: () => {
  ...
  const renderCategory = (category: Category) => {
    ...
    return (
      <div key={category.id} className="space-y-2">
        ...
        {children.length > 0 && (
          <div className="ml-8 space-y-2">{children.map(renderCategory)}</div>
        )}
      </div>
    );
  };
  ...
  const renderCategoryList = (categoryList: Category[], type: "income" | "expense") => {
    ...
    return <div className="space-y-2">{rootCategories.map(renderCategory)}</div>;
  };
      ...
      <Main>
        ...
        {categories.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-muted-foreground mb-4">{t("page.categories.create_prompt")}</p>
            <Button onClick={() => setShowForm(true)}>
              <Plus className="mr-2 h-4 w-4" />
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            {(filterType === "all" || filterType === "income") && (
              <Card>
                ...
                <CardContent>{renderCategoryList(incomeCategories, "income")}</CardContent>
              </Card>
            )}
            ...
            {(filterType === "all" || filterType === "expense") && (
              <Card>
                ...
                <CardContent>{renderCategoryList(expenseCategories, "expense")}</CardContent>
              </Card>
            )}
          </div>
        )}
        ...
        <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {t("page.categories.delete_description", {
                  name: selectedCategory ? getCategoryName(selectedCategory) : "",
                })}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
