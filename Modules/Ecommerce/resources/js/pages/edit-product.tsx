...
import { ProductCategory, ProductTag, ProductFormData, Product } from "@/types/ecommerce"
...
export default function EditProduct({ product, categories = [], tags = [] }: EditProductPageProps) {
  ...
  const handleFeaturedImageChange = (files: File[]) => {
    ...
    if (files.length > 0) setRemoveFeaturedImage(false)
  }
  ...
  const handleSubmit = (status?: 'draft' | 'active') => {
    ...
    router.post(route('dashboard.ecommerce.products.update', product?.slug || ''), formData, {
      ...
      onSuccess: () => {
        setProcessing(false)
        toast({
          title: t('page.ecommerce.products.toast.update_success'),
          description: t('page.ecommerce.products.toast.update_success_description'),
        })
      },
      onError: (errors) => {
        setProcessing(false)
        ...
        toast({
          variant: "destructive",
          title: t('page.ecommerce.products.toast.update_error'),
  ...
  return (
    <>
      <AuthenticatedLayout title={t('page.ecommerce.products.edit.title', { name: product?.name || t('common.fields.product') })}>
        <Main>
            ...
            <div className="grid flex-1 auto-rows-max gap-4">
              <div className="flex items-center gap-4">
                ...
                <h1 className="flex-1 shrink-0 whitespace-nowrap text-xl font-semibold tracking-tight sm:grow-0">
                  {t('page.ecommerce.products.edit.heading')}
                </h1>
                <div className="hidden items-center gap-2 md:ml-auto md:flex">
                  <Button variant="outline" size="sm" onClick={() => handleSubmit('draft')} disabled={processing}>
                    {t('page.ecommerce.products.edit.save_draft')}
                  </Button>
                  <Button size="sm" onClick={() => handleSubmit()} disabled={processing}>
                    {processing ? t('page.ecommerce.products.edit.updating') : t('page.ecommerce.products.edit.submit')}
                  </Button>
                </div>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-[1fr_350px] lg:gap-8">
                <div className="grid auto-rows-max items-start gap-4 lg:gap-8">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>{t('page.ecommerce.products.form.edit_details_description')}</CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                <div className="grid auto-rows-max items-start gap-4 lg:gap-8">
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <MediaUploader name="featured_image" initialMedia={initialMedia} multiple={false} maxFiles={1}
                        acceptedFileTypes={['image/jpeg', 'image/png', 'image/webp', 'image/jpg']}
                        maxFileSize={5} onChange={handleFeaturedImageChange} onRemove={handleRemoveMedia} />
                    </CardContent>
                  </Card>
              ...
              <div className="flex items-center justify-center gap-2 md:hidden">
                <Button variant="outline" size="sm" onClick={() => handleSubmit('draft')} disabled={processing}>{t('page.ecommerce.products.edit.save_draft')}</Button>
                <Button size="sm" onClick={() => handleSubmit()} disabled={processing}>{processing ? t('page.ecommerce.products.edit.updating') : t('page.ecommerce.products.edit.submit')}</Button>
              </div>
            </div>
          </div>
        </Main>
